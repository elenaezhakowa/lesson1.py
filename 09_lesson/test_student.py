...
def test_get_student():
    ...
    """
    # Получаем список студентов из API
    response = requests.get(f"{api.url}/students")
    if response.status_code != 200:
        pytest.fail(f"""Failed request: Status Code {response.status_code}.
                    Response content: {response.content.decode('utf-8')}""")
    try:
        api_result = response.json()
    except requests.exceptions.JSONDecodeError:
        pytest.fail(f"Invalid JSON format: {response.content.decode('utf-8')}")
